/**
 * Server with WhatsApp Cloud API Integration
 * This is an alternative to server.ts that uses Meta's official WhatsApp Cloud API
 * instead of Baileys (unofficial library)
 */
...
// WhatsApp State
let whatsappClient: WhatsAppCloudAPI | null = null;
...
async function initializeWhatsAppClient() {
  try {
    if (!process.env.WHATSAPP_PHONE_NUMBER_ID || !process.env.WHATSAPP_ACCESS_TOKEN) {
      console.warn('WhatsApp Cloud API credentials not found. Please set WHATSAPP_PHONE_NUMBER_ID and WHATSAPP_ACCESS_TOKEN');
      connectionStatus = 'error';
      return;
    }

    whatsappClient = new WhatsAppCloudAPI(
    ...
    console.log('WhatsApp Cloud API client initialized successfully');
  } catch (error) {
    console.error('Failed to initialize WhatsApp Cloud API client:', error);
    connectionStatus = 'error';
  }
}
  ...
  try {
    ...
    for (const item of numbers) {
      ...
      try {
        ...
        console.log(`Sending message to ${item.phone}...`);

        // Send via WhatsApp Cloud API
        const messageId = await whatsappClient.sendMessage(item.phone, messageText);

        console.log(`Message sent successfully to ${item.phone}. Message ID: ${messageId}`);
        ...
        console.log(`Status updated for ${item.phone}. Waiting ${Math.round(delay / 1000)}s...`);
    ...
    try {
      ...
      if (pathname === '/api/whatsapp') {
        ...
        if (action === 'connect') {
          ...
          res.statusCode = 200;
          res.end(JSON.stringify({ message: 'WhatsApp Cloud API connection initialized' }));
          return;
        }
      ...
      // Expose client globally for Next.js API routes to use
      (global as any).whatsappClient = whatsappClient;
      (global as any).whatsappConnectionStatus = connectionStatus;
